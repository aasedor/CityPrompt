"""Regression contracts for the ten reference-locked Wave 14 siblings."""
...
MEMORY_VERSION = "2026-08-01-wave14-sibling-variant-lock-v113"
REQUIRED_CHANNELS = {
    "albedo",
    "normal",
    "roughness",
...
IDENTITY_NODES = {
    "art-deco-black-chrome-tower": {"BLACK_EntranceDepth", "BLACK_CrownFlute_0.0"},
    "art-deco-polychrome-zigzag-tower": {"POLY_PortalArch", "POLY_GreenPyramid"},
    "mid-century-wood-stone-pavilion": {"WOODSTONE_MainHearth", "WOODSTONE_ContinuousHipRoof"},
    "mid-century-white-brise-soleil-pavilion": {"WHITE_FrontScreen_Vertical_0", "WHITE_RoofGardenRoom_Front_Glass"},
    "modern-black-screen-machiya": {"MODMACHIYA_PassageDepth", "MODMACHIYA_RoofGardenFront_Slat_0"},
    "red-machiya-cafe-gallery": {"CAFE_SidePassage", "CAFE_KawaraRoof_ContinuousGable"},
    "dark-frame-clear-glass-office": {"DARKOFFICE_LobbyVoid", "DARKOFFICE_GreenhouseRoof"},
    "mass-timber-glass-office": {"TIMBEROFFICE_WestSawtoothMetal_0", "TIMBEROFFICE_InteriorTreeCrown_0"},
    "streamline-moderne-theater": {"STREAM_CurvedLobbyGlassLeft", "STREAM_IntegralFin"},
    "egyptian-revival-theater": {"EGYPT_LeftPylon", "EGYPT_ContinuousCornice"},
}
...
def test_wave14_manifest_is_variant_specific_and_size_flexible(family: str):
    ...
    assert set(compatibility["profiles"]) == set(compatibility["preferredProfiles"])
    assert compatibility["minimumPreferredProfiles"] == len(compatibility["preferredProfiles"])
    ...
    assert {item["variant_key"] for item in modules if item["role"] == "floor"} == {
...
@pytest.mark.parametrize("family", FAMILIES)
def test_wave14_custom_skin_and_generation_provenance_are_complete(family: str):
    spec = FAMILIES[family]
    root = FAMILIES_ROOT / family
    ...
    assert provenance["design_lock"]["native_envelope_dimensions_m"] == list(spec["native"])
...
@pytest.mark.parametrize("family", FAMILIES)
def test_wave14_glb_keeps_identity_and_layered_window_materials(family: str):
    spec = FAMILIES[family]
    root = FAMILIES_ROOT / family
    ...
    assert any(item.get("wave14_variant_specific_material") is True for item in material_extras)


@pytest.mark.parametrize("family", FAMILIES)
def test_wave14_review_assets_signature_catalogue_and_assessment_bind(family: str):
    spec = FAMILIES[family]
    root = FAMILIES_ROOT / family
    for role in (
        "preview",
    ...
    assert (FAMILIES_ROOT / f"wave14-{family}-comparison.jpg").is_file()
    ...
    thumbnail = (
        ...
        / spec["catalogue_slug"] / f"variant_{spec['catalogue_variant_index']}.png"
    )
