"""Create a phone-readable reference/model comparison for Wave 10 townhomes."""
...
def main() -> int:
    ...
    rows = [
        ...
        (
            panel(source / "aerial-source-v1.png", "ROOF / MASSING GOALPOST"),
            panel(root / f"{FAMILY}_aerial.png", "MODEL - FOUR-TERRACE ROOF PROOF"),
        ),
        (
            panel(source / "rear-elevation-source-v1.png", "LANE SERVICE GOALPOST"),
            panel(
                root / f"{FAMILY}_rear_corner_oblique.png",
                "MODEL - GARAGES / REAR OCCUPATION",
            ),
        ),
        (
            panel(
                source / "elevation-source-v1.png",
                "WINDOW / ENTRY DETAIL GOALPOST",
                centering=(0.31, 0.69),
            ),
            panel(
                root / f"{FAMILY}_identity_close.png",
                "MODEL - PHYSICAL CAVITIES / LOW-E GLASS",
                centering=(0.46, 0.57),
            ),
        ),
    ]
    ...
    ImageDraw.Draw(sheet).text(
        (GAP, 19),
        "Cedar & Black-Metal Townhomes - reference-locked LEGO family",
        fill="#111820",
        font=font(36),
    )
    y = title_height + GAP
    ...
    combined = ROOT / "wave10-townhome-comparison.jpg"
