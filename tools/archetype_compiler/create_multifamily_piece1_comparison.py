"""Create phone-readable reference/model sheets for multifamily Piece 1."""
...
def create_sheet(family: str) -> Path:
    ...
    rows = [
        (
            panel(source / "street-hero-source-v1.png", "REFERENCE - SIX-STOREY MASSING / PUBLIC IDENTITY"),
            panel(root / f"{family}_preview.png", "MODEL - COMPLETE FIXED LANDMARK"),
        ),
        (
            panel(source / "front-elevation-source-v1.png", "REFERENCE - BROAD TIMBER BAYS / CARVED LOBBY"),
            panel(root / f"{family}_front_elevation.png", "MODEL - PHYSICAL FRONT ELEVATION"),
        ),
        (
            panel(source / "aerial-roof-source-v1.png", "REFERENCE - SEDUM / PV / TERRACE ROOF"),
            panel(root / f"{family}_aerial.png", "MODEL - PHYSICAL ROOF SYSTEMS"),
        ),
        (
        ...
        (
            panel(source / "material-construction-source-v1.png", "REFERENCE - CUSTOM MATERIAL / JOINT LOCK"),
            panel(root / f"{family}_facade_close.png", "MODEL - PBR SKIN / PANE / FRAME / LOAD PATH"),
        ),
    ]
    title_height = 86
    ...
    ImageDraw.Draw(sheet).text(
        (GAP, 19),
        f"{spec['label']} - multifamily completion Piece 1 pilot",
        fill="#111820",
        font=font(34),
    )
    y = title_height + GAP
    ...
    root_copy = ROOT / f"multifamily-piece1-{family}-comparison.jpg"
    sheet.save(root_copy, quality=91, optimize=True, progressive=True)
    print(f"[multifamily-piece1-comparison] {destination.relative_to(REPO)}")
    return destination
