"""Create phone-readable reference/model comparison sheets for Wave 15."""
...
def create_sheet(family: str) -> Path:
    ...
    rows = [
        (
            panel(source / "street-hero-source-v1.png", "REFERENCE - HERO MASSING / PROGRAM IDENTITY"),
    ...
    title = f"{spec['label']} - reference-locked program-diverse LEGO family"
    ...
    root_copy = ROOT / f"wave15-{family}-comparison.jpg"
    sheet.save(root_copy, quality=91, optimize=True, progressive=True)
    print(f"[wave15-comparison] {destination.relative_to(REPO)}")
    return destination
...
def create_overview(families: list[str]) -> Path:
    ...
    ImageDraw.Draw(sheet).text(
        (GAP, 20),
        "Wave 15 - ten program-diverse reference-locked LEGO landmarks",
        fill="#111820",
        font=font(34),
    )
    ...
    destination = ROOT / "wave15-program-diversity-overview.jpg"
    sheet.save(destination, quality=91, optimize=True, progressive=True)
    print(f"[wave15-comparison] {destination.relative_to(REPO)}")
    return destination
