...
def _add_signature_podium_details(grammar: dict, parts: list, width: float, depth: float,
    ...
    if "shopfront_canopies" in kits:
        ...
        if "porte_cochere" in kits:
            ...
            _signature_relief_box(
                parts, "Signature_TheaterMarqueeSlab", (canopy_w, canopy_d, 0.24),
                (0, front_y - canopy_d / 2, canopy_z), warm, bevel_m=0.08,
            )
            _signature_relief_box(
                parts, "Signature_TheaterMarqueeFascia", (canopy_w + 0.34, 0.20, 0.44),
                (0, front_y - canopy_d - 0.08, canopy_z - 0.04), metal, bevel_m=0.06,
            )
            for bulb_index in range(13):
                bulb_x = -canopy_w * 0.46 + canopy_w * 0.92 * bulb_index / 12
                _signature_relief_box(
                    parts, f"Signature_TheaterMarqueeBulb{bulb_index:02d}",
                    (0.12, 0.08, 0.12),
                    (bulb_x, front_y - canopy_d - 0.19, canopy_z - 0.04),
                    mats["interior_warm"], bevel_m=0.035,
                )
        else:
            _signature_relief_box(
    ...
    if kits & {"sign_frieze"}:
        if "deco_portal" in kits:
            frieze_w = width * 0.78
            frieze_z = height * 0.77
            _signature_relief_box(
                parts, "Signature_TheaterSignFrieze",
                (frieze_w, 0.24, 0.70),
                (0, front_y - 0.15, frieze_z), warm, bevel_m=0.055,
            )
            for suffix, trim_z in (("Top", frieze_z + 0.31), ("Bottom", frieze_z - 0.31)):
                _signature_relief_box(
                    parts, f"Signature_TheaterSignTrim{suffix}",
                    (frieze_w + 0.20, 0.28, 0.09),
                    (0, front_y - 0.18, trim_z), metal, bevel_m=0.025,
                )
        else:
            parts.append(add_box("Signature_SignFrieze", (width * 0.82, 0.18, 0.62),
                                 (0, front_y - 0.12, height * 0.77), mats["secondary"]))

    if "vertical_blade_sign" in kits:
        # Keep the theater blade on a structural pier instead of covering the
        # render-locked glazing. The podium owns this fixed public-facing
        # identity; repeatable floor modules must never multiply it.
        sign_x = -width * 0.20
        sign_w = min(1.45, width * 0.075)
        sign_h = min(height * 0.80, 4.6)
        sign_z = min(height - sign_h / 2 - 0.12, height * 0.56)
        sign_y = front_y - 0.82
        _signature_relief_box(
            parts,
            "Signature_TheaterBladeFace",
            (sign_w, 1.42, sign_h),
        ...
        for edge_index, edge_x in enumerate(
            (sign_x - sign_w / 2 + 0.08, sign_x + sign_w / 2 - 0.08)
        ):
            _signature_relief_box(
                parts,
                f"Signature_TheaterBladeTrim{edge_index}",
                (0.09, 1.50, sign_h * 0.93),
                (edge_x, sign_y - 0.02, sign_z),
                metal,
                bevel_m=0.025,
            )
        _signature_relief_box(
            parts,
            "Signature_TheaterBladeCrown",
            (sign_w * 1.22, 1.50, 0.30),
            (sign_x, sign_y - 0.02, sign_z + sign_h / 2 - 0.04),
            metal,
            bevel_m=0.055,
        )
...
def _add_signature_floor_details(grammar: dict, parts: list, width: float, depth: float,
    ...
    if crown and "vertical_blade_sign" in kits:
        # The upper fixed crown owns the tall readable portion of the theater
        # blade. Ordinary floor variants never receive it, so height changes
        # cannot multiply the public sign.
        sign_x = -width * 0.20
        sign_w = min(1.55, width * 0.08)
        sign_h = height * 0.86
        sign_y = facade_y - 0.84
        _signature_relief_box(
            parts, "Signature_TheaterCrownBladeFace",
            (sign_w, 1.46, sign_h),
            (sign_x, sign_y, height * 0.50), warm, bevel_m=0.11,
        )
        for edge_index, edge_x in enumerate(
            (sign_x - sign_w / 2 + 0.08, sign_x + sign_w / 2 - 0.08)
        ):
            _signature_relief_box(
                parts, f"Signature_TheaterCrownBladeTrim{edge_index}",
                (0.10, 1.54, sign_h * 0.94),
                (edge_x, sign_y - 0.02, height * 0.50), metal, bevel_m=0.028,
            )
        _signature_relief_box(
            parts, "Signature_TheaterCrownBladeCap",
            (sign_w * 1.22, 1.54, 0.34),
            (sign_x, sign_y - 0.02, height * 0.50 + sign_h / 2 - 0.05),
            metal, bevel_m=0.06,
        )
...
def _add_signature_roof_details(grammar: dict, parts: list, width: float, depth: float,
    ...
    if "fly_tower" in kits:
        # A stage house is a roof-level occupied volume, not generic rooftop
        # mechanical clutter. Keep it behind the decorative street facade and
        # wholly inside the roof module's metric extent.
        tower_w = width * 0.56
        tower_d = depth * 0.58
        tower_h = max(0.8, height - base_z - 0.22)
        tower_y = depth * 0.12
        _signature_relief_box(
            parts,
            "Signature_TheaterFlyTower",
            (tower_w, tower_d, tower_h),
            (0, tower_y, base_z + tower_h / 2),
            mats["primary"],
            bevel_m=0.12,
        )
        cap_h = min(0.30, tower_h * 0.10)
        _signature_relief_box(
            parts,
            "Signature_TheaterFlyTowerCap",
            (tower_w + 0.34, tower_d + 0.34, cap_h),
            (0, tower_y, base_z + tower_h - cap_h / 2),
            stone,
            bevel_m=0.055,
        )
        # Three shallow stepped fins make the service volume belong to the
        # Art Deco composition while preserving a quiet rear elevation.
        for fin_index, scale in enumerate((0.72, 0.52, 0.34)):
            fin_h = tower_h * (0.38 - fin_index * 0.065)
            fin_y = tower_y - tower_d / 2 - 0.10
            _signature_relief_box(
                parts,
                f"Signature_TheaterFlyTowerFin{fin_index}",
                (tower_w * scale, 0.18, fin_h),
                (0, fin_y, base_z + tower_h - fin_h / 2 - cap_h),
                warm if fin_index == 2 else stone,
                bevel_m=0.035,
            )
...
def render_presentation_views(
    ...
    views = (
        ...
        *((
        ...
        ("front_corner_oblique", (dist * 0.74, -dist * 0.92, focus_height * 0.64),
         (0.0, 0.0, focus_height * 0.43), 46),
        ("rear_corner_oblique", (-dist * 0.72, dist * 0.92, focus_height * 0.68),
         (0.0, 0.0, focus_height * 0.45), 47),
        ("aerial", (dist * 0.62, -dist * 0.78, focus_height + dist * 0.52), (0.0, 0.0, focus_height * 0.38), 49),
        ("facade_close", (0.0, -(depth / 2 + max(27.0, focus_height * 1.35)), focus_height * 0.42),
         (0.0, -depth * 0.10, focus_height * 0.43), 58),
...
def generate(grammar: dict, output: Path, *, floors_override: int | None, keep_blend: bool,
    ...
    manifest = {
        ...
        "archetype_aliases": grammar.get("archetype_aliases", []),
        ...
        "dimensions": dims,
        "native_width_m": dims["width_m"],
        "native_depth_m": dims["depth_m"],
        "native_floors": dims["default_floors"],
