"""Create a phone-readable reference/model comparison for Wave 10 aquatic centre."""
...
def main() -> int:
    ...
    rows = [
        (
            panel(
                ...
                "CANONICAL DOUBLE-WAVE NATATORIUM GOALPOST",
            ),
            panel(
                root / f"{FAMILY}_preview.png",
                "MODEL - MATCHED PUBLIC CORNER / COMPLETE SHELL",
            ),
        ),
        (
            panel(
                source / "front-elevation-source-v1.png",
                "RECTIFIED WAVE / SUPPORT / ENTRY GOALPOST",
            ),
            panel(
                root / f"{FAMILY}_street.png",
                "MODEL - TWO CRESTS / SADDLE / CLEAR ENTRY",
            ),
        ),
        (
            panel(
                source / "aerial-roof-source-v1.png",
                "STANDING-SEAM / MAST / SKYLIGHT GOALPOST",
            ),
            panel(
                root / f"{FAMILY}_aerial.png",
                "MODEL - CONTINUOUS SHELL / EXACTLY TWO MASTS",
            ),
        ),
        (
            panel(
                source / "rear-service-source-v1.png",
                "POLYCARBONATE / COBALT SERVICE GOALPOST",
            ),
            panel(
                root / f"{FAMILY}_rear_corner_oblique.png",
                "MODEL - DISTINCT RIBBED SERVICE ELEVATION",
            ),
        ),
        (
            panel(
                source / "pool-occupied-depth-source-v1.png",
                "OCCUPIED TIMBER POOL-HALL GOALPOST",
            ),
            panel(
                root / f"{FAMILY}_pool_close.png",
                "MODEL - PHYSICAL LOW-IRON GLASS / POOL DEPTH",
            ),
        ),
        (
            panel(
                source / "material-construction-source-v1.png",
                "TIMBER / CONCRETE / GLASS JUNCTION GOALPOST",
            ),
            panel(
                root / f"{FAMILY}_structure_close.png",
                "MODEL - INTEGRATED SUPPORT / WAVE SOFFIT",
            ),
        ),
    ]
    title_height = 86
    ...
    ImageDraw.Draw(sheet).text(
        (GAP, 19),
        "Parametric Wave-Shell Aquatic Centre - reference-locked LEGO family",
        fill="#111820",
        font=font(36),
    )
    y = title_height + GAP
    ...
    combined = ROOT / "wave10-aquatic-comparison.jpg"
