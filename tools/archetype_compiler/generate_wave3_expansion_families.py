...
FAMILIES = {
    ...
    "barcelona-mercat": {
        ...
        "identity": (
            ...
            "entrance arch, polychrome stained glass, tiled perimeter arcade, "
            "open produce hall and ornate ridge cresting."
        ),
        "materials": (
    ...
    "historic-grand-station": {
        ...
        "identity": (
            "A monumental pale-granite Beaux-Arts terminal with three deeply "
            "carved arched portals, paired Corinthian columns, a sculpted clock "
            "attic, and three immense iron-and-glass barrel-vault train sheds."
        ),
        "materials": (
...
def _concert_lobby_top(x: float) -> float:
    return 24.0 + 10.0 * max(0.0, 1.0 - (x / 34.0) ** 2) ** 0.55
...
def concert_lobby(
    ...
    columns, rows = 20, 5
    x_values = [-34.0 + 68.0 * i / columns for i in range(columns + 1)]
    ...
    objects.append(
        vertical_shell_panel(
            ...
            lambda u: 29.0 + 5.8 * math.sin(math.pi * u),
            mats["dark"],
            thickness=0.18,
            u_steps=20,
            v_steps=5,
    ...
    for level in (7.0, 12.5, 18.0, 23.2):
        objects.append(
            box(
                ...
                (43.0 - level * 0.35, 4.8, 0.32),
                (0, -25.8 + level * 0.025, level),
                mats["timber"],
                0.08,
            )
        )
        ...
        objects.append(
            box(
                ...
                (42.0 - level * 0.35, 0.09, 1.05),
                (0, -28.0 + level * 0.025, level + 0.62),
                mats["glass"],
            )
        )
    for index, x in enumerate((-25, -18, -11, 0, 11, 18, 25)):
    ...
    for index in range(count):
        ...
        objects.append(
            box(
                ...
                (width, depth, rise),
                (0, -31.0 - index * depth, rise / 2 + index * rise),
                mats["stone"],
                0.025,
            )
        )
...
def concert_side_glazing(
    ...
    for side in (-1, 1):
        x_pane = side * 43.42
        x_back = side * 42.72
        for row, z in enumerate((9.4, 17.4, 25.4)):
            height = 2.45
            objects.append(
                box(
                    f"CONCERT_SideGlassRibbon_{side}_{row}",
                    (0.14, 52.0, height),
                    (x_pane, 3.0, z),
                    mats["glass"],
                )
            ...
            objects.append(
                box(
                    f"CONCERT_SideOccupiedRibbon_{side}_{row}",
                    (0.10, 50.8, height - 0.36),
                    (x_back, 3.0, z),
                    mats["dark"],
                )
            )
            for bay, y in enumerate(range(-23, 30, 4)):
                objects.append(
                    box(
                        f"CONCERT_SideMullion_{side}_{row}_{bay}",
                        (0.22, 0.09, height + 0.28),
                        (side * 43.54, y, z),
                        mats["bronze"],
                    )
                )
...
def concert_fixed(
    ...
    objects.append(
        vertical_ribbon_panel(
            ...
            -45.5,
            45.5,
            lambda u: -29.18 + 0.25 * math.sin(math.pi * u),
            lambda u: (
                13.0
                + 10.5 * math.sin(math.pi * u) ** 1.35
            ),
            lambda u: (
                16.2
                + 11.0 * math.sin(math.pi * u) ** 1.35
    ...
    for side in (-1, 1):
        x0, x1 = (-45.0, -11.5) if side < 0 else (11.5, 45.0)
        objects.append(
            vertical_ribbon_panel(
    ...
    objects.append(
        surface_volume(
            ...
            lambda u, v: (
                ...
                -7.0 + 39.0 * v,
                25.0
                + 5.0 * math.sin(math.pi * u)
                + 4.0 * v
                + 16.0
                * math.exp(-((u - 0.68) / 0.19) ** 2)
                * (0.30 + 0.70 * v ** 1.4)
                - 3.0 * math.exp(-((u - 0.30) / 0.16) ** 2) * v,
            ),
            mats["shell"],
            u_steps=42,
            v_steps=20,
    ...
    objects.append(
        vertical_ribbon_panel(
            ...
            41.0,
            lambda u: -7.15,
            lambda u: 22.4 + 4.6 * math.sin(math.pi * u),
            lambda u: 25.0 + 5.0 * math.sin(math.pi * u),
            mats["shell"],
            thickness=0.70,
            u_steps=40,
            v_steps=3,
    ...
    objects.append(
        surface_volume(
            ...
            lambda u, v: (
                ...
                8.0 + v * 25.0,
                31.0
                + 8.0 * math.sin(math.pi * u)
                + 7.0 * v
                + 5.5 * math.exp(-((u - 0.28) / 0.17) ** 2) * v,
            ),
            mats["shell"],
            u_steps=34,
            v_steps=16,
    ...
    # Long side ribbons make the curved envelope continuous on both elevations.
    for side in (-1, 1):
        for band_index, (z0, z1) in enumerate(((3.6, 7.8), (11.4, 15.7), (19.4, 23.7))):
            x0 = side * 44.0
            x1 = side * 43.2
            points = []
            uv = []
            segments = 24
            for shell_x in (x0, x1):
                for row in (0, 1):
                    for index in range(segments + 1):
                        t = index / segments
                        y = -23.0 + 54.0 * t
                        wave = 1.15 * math.sin(math.pi * t + band_index * 0.7)
                        z = (z0 if row == 0 else z1) + wave
                        points.append((shell_x, y, z))
                        uv.append((t * 2.8, float(row)))
            # index order: outer lower, outer upper, inner lower, inner upper.
            stride = segments + 1
            faces = []
            for index in range(segments):
                ol, ou = index, stride + index
                il, iu = stride * 2 + index, stride * 3 + index
                faces.extend(
                    [
                        (ol, ol + 1, ou + 1, ou),
                        (il, iu, iu + 1, il + 1),
            ...
            objects.append(
                mesh_object(
                    f"CONCERT_SideShell_{side}_{band_index}",
                    points,
                    faces,
                    mats["shell"],
                    uv,
...
def mercat_fixed(
    ...
    # Five real roof aisles: clear panels, ridge/eave members, transverse
    # trusses and clerestory ventilation.  The centre nave is widest/tallest.
    aisle_centres = (-26.0, -13.0, 0.0, 13.0, 26.0)
    aisle_widths = (11.0, 12.0, 14.0, 12.0, 11.0)
    ridge_heights = (16.0, 17.5, 20.0, 17.5, 16.0)
    eave_heights = (11.0, 11.5, 12.0, 11.5, 11.0)
    ...
    for aisle, (cx, width, ridge_z, eave_z) in enumerate(
        ...
        objects.append(
            quad_panel(
                f"MERCAT_RoofGlassRight_{aisle}",
                (
                    (cx, roof_front, ridge_z),
                    (cx + half, roof_front, eave_z),
                    (cx + half, roof_rear, eave_z),
        ...
        # Clerestory louver reads as a separate ventilating construction band.
        objects.append(
            box(
                f"MERCAT_Clerestory_{aisle}",
                (width * 0.52, roof_rear - roof_front, 1.25),
                (cx, 0.6, eave_z + 1.0),
                mats["roof_glass"],
            )
        )
        for y in range(-18, 21, 3):
            objects.append(
                box(
                    f"MERCAT_ClerestoryMullion_{aisle}_{y}",
                    (width * 0.53, 0.08, 1.38),
                    (cx, y, eave_z + 1.0),
                    mats["iron"],
                )
            )
    ...
    # Gable frame and custom iron crest/scrolls.
    objects.extend(
        [
            beam(
                "MERCAT_GableLeft",
...
def station_fixed(
    ...
    # The head house is assembled around three empty portal volumes.  Its core
    # stops five metres behind the public facade, preserving a real coffered
    # cavity for glass, doors and occupied backing.
    wall_intervals = (
        (-98.0, -63.0),
        (-33.0, -18.0),
        (18.0, 33.0),
    ...
    def station_portal(
        ...
        *,
        ...
        # Each stair is seated into the same granite plinth.
        for step in range(9):
            rise = 0.18
            depth = 0.72
            parts.append(
    ...
    objects.extend(station_portal("STATION_CentralPortal", 0.0, 34.0, 32.0, stair_width=36.0))
    objects.extend(station_portal("STATION_LeftPortal", -48.0, 28.0, 28.5, stair_width=30.0))
    objects.extend(station_portal("STATION_RightPortal", 48.0, 28.0, 28.5, stair_width=30.0))

    # Paired giant-order columns bracket every portal and continue the exact
    # Beaux-Arts structural rhythm across the head house.
    column_positions = (-66.5, -62.8, -34.2, -30.5, -19.0, -15.3, 15.3, 19.0, 30.5, 34.2, 62.8, 66.5)
    ...
    for group_x in (-49.0, 49.0):
        ...
        for figure in range(5):
            x = group_x + (figure - 2) * 2.0
            figure_height = 4.0 + (2 - abs(figure - 2)) * 0.65
            objects.extend(
                [
                    cylinder(
                        ...
                        0.62,
                        figure_height * 0.60,
                        (x, -39.0, 42.0 + figure_height * 0.30),
                        mats["relief"],
                        16,
                    ),
                    sphere(
                        ...
                        0.52,
                        (x, -39.0, 42.0 + figure_height * 0.68),
                        mats["relief"],
                        (1.0, 0.92, 1.08),
                        16,
                        8,
    ...
    objects.extend(barrel_vault("STATION_LeftShed", -66.0, 62.0, 20.5))
    objects.extend(barrel_vault("STATION_CentralShed", 0.0, 66.0, 20.5))
    objects.extend(barrel_vault("STATION_RightShed", 66.0, 62.0, 20.5))
