"""Generate archetype-owned PBR skins for park LEGO batch 3."""
...
SCHEDULE_PATH = Path(__file__).with_name("park_archetype_batch3_skin_sources.json")
...
def main() -> None:
    ...
    if args.dry_run:
        print(
            f"archetypes={len(schedule['archetypes'])} roles=6 "
            "api_calls=0 source_pixels_projected=false"
        )
        return
