"""Generate the ten archetype-owned PBR skin sets in park LEGO batch 4."""
...
SCHEDULE_PATH = Path(__file__).with_name("park_archetype_batch4_skin_sources.json")
...
def main() -> None:
    ...
    if errors:
        raise SystemExit("\n".join(errors))
    if args.dry_run:
        print("archetypes=10 roles=6 api_calls=0 source_pixels_projected=false")
        return
