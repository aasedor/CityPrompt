"""Render one mobile-friendly batch-4 reference/skin comparison sheet."""
...
SCHEDULE = Path(__file__).with_name("park_archetype_batch4_skin_sources.json")
DEFAULT_OUT = REPO_ROOT / "docs/park_lego_batch4/park_lego_batch4_reference_skin_sheet.jpg"
...
def main() -> None:
    ...
    draw.text((34, 24), "PARK LEGO BATCH 4 — ARCHETYPE REFERENCE + STATIONARY PBR SKINS", fill="white", font=font)
    draw.text((34, 50), "10 exact v0 programs • no people • no large buildings • zero API calls • source pixels never projected", fill="#c8d7c6", font=font)
    ...
    for index, (slug, item) in enumerate(items):
        ...
        for role_index, role in enumerate(roles):
            ...
            if image_path.exists():
                swatch = cover(Image.open(image_path).convert("RGB"), (142, 142))
            else:
                swatch = Image.new("RGB", (142, 142), ROLE_COLORS[role])
