"""Render the mobile-friendly Batch 11 reference/skin comparison sheet."""
...
SCHEDULE=Path(__file__).with_name("park_archetype_batch11_skin_sources.json")
DEFAULT_OUT=REPO_ROOT/"docs/park_lego_batch11/park_lego_batch11_reference_skin_sheet.jpg"
...
def main()->None:
    ...
    items=list(json.loads(SCHEDULE.read_text(encoding="utf-8"))["archetypes"].items()); width,row_height=1500,245; canvas=Image.new("RGB",(width,110+row_height*len(items)),"#eee9df"); draw=ImageDraw.Draw(canvas); font=ImageFont.load_default(); draw.rectangle((0,0,width,90),fill="#252b29"); draw.text((34,24),"PARK LEGO BATCH 11 - CITY REFERENCE + ADAPTIVE 3D FAMILY",fill="white",font=font); draw.text((34,50),"10 previously uncovered parents - park evidence only - no people/buildings - zero API calls",fill="#c8d7c6",font=font)
