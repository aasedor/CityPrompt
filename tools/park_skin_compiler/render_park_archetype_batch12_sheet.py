"""Render the mobile-friendly Batch 12 reference/skin comparison sheet."""
...
SCHEDULE=Path(__file__).with_name("park_archetype_batch12_skin_sources.json")
DEFAULT_OUT=REPO_ROOT/"docs/park_lego_batch12/park_lego_batch12_reference_skin_sheet.jpg"
...
def main()->None:
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument("--out",type=Path,default=DEFAULT_OUT);parser.add_argument("--skin-root",type=Path,default=REPO_ROOT/"frontend/public/park-skins");args=parser.parse_args()
    items=list(json.loads(SCHEDULE.read_text(encoding="utf-8"))["archetypes"].items());width,row_height=1500,245;canvas=Image.new("RGB",(width,110+row_height*len(items)),"#eee9df");draw=ImageDraw.Draw(canvas);font=ImageFont.load_default();draw.rectangle((0,0,width,90),fill="#252b29");draw.text((34,24),"PARK LEGO BATCH 12 - EXACT REFERENCE + ADAPTIVE 3D FAMILY",fill="white",font=font);draw.text((34,50),"10 civic, event and water parents - park evidence only - no people/buildings - zero API calls",fill="#c8d7c6",font=font)
    roles=["paver","lawn","asphalt","planting","safety","timber"]
    for index,(slug,item) in enumerate(items):
        top=100+index*row_height;draw.rounded_rectangle((20,top,width-20,top+row_height-10),radius=16,fill="#f8f5ee",outline="#c7bfae",width=2);source=Image.open(REPO_ROOT/item["source"]).convert("RGB");canvas.paste(cover(source,(420,190)),(40,top+28));draw.text((40,top+8),f"{index+1:02d}  {slug.replace('-',' ').upper()}",fill="#27312d",font=font)
