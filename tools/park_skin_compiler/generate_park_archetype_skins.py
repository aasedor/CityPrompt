"""Generate five archetype-calibrated, stationary PBR park skin libraries."""
...
SCHEDULE_PATH = Path(__file__).with_name("park_archetype_skin_sources.json")
DEFAULT_OUT = REPO_ROOT / "artifacts/neighborhood-park-lego-depth-pilot/five-archetype-skins-v1"
...
def generate(schedule: dict, output_root: Path) -> None:
    defaults = schedule["materialDefaults"]
    for archetype_index, (slug, item) in enumerate(schedule["archetypes"].items()):
