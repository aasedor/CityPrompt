...
def boards(root, manifest):
    ...
    label(board,22,15,'LOCKED CATALOGUE SOURCES / Exact building pilot',28)
    ...
    label(phone,28,25,'SOURCE AND ARCHITECTURAL CLAY',32)
    ...
    for i,role in enumerate(('architecture_close','glass_close','facade_close','roof_contact',manifest['mandatory_review_views'][-1],'rear_side')):
...
def verify(root, manifest):
    ...
    # GLB is Y-up; authoring/report coordinates are Z-up. Validate the complete
    # delivered envelope so overhangs and asymmetric projections cannot be lost.
    native=report['native_dimensions_m']
    checks['native envelope measured from GLB']=all(abs(float(scene.extents[i])-native[key])<1e-4 for i,key in ((0,'width'),(1,'height'),(2,'depth')))
...
def main():
    ...
    # A still-running build must not leave half-written immutable proof boards.
    required=[root/'build-report.json']
    required += [root/s['path'] for s in manifest['source_contract']['sources']]
    required += [root/'renders'/f'{v}.png' for v in manifest['mandatory_review_views']]
    missing=[str(p) for p in required if not p.is_file()]
    if missing:raise FileNotFoundError('Build evidence is not complete: '+', '.join(missing))
