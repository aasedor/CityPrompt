...
async def fetch_streetview_plate(
    lat: float, lng: float, heading: float, fov: float, width: int, height: int,
) -> dict | None:
    ...
    try:
        async with httpx.AsyncClient(timeout=25.0) as client:
            ...
            r = await client.get(
                ...
                params={"pano": pano, "size": f"{w}x{h}", "heading": round(heading),
                        "fov": round(fov), "pitch": 0, "key": key},
            )
    ...
    try:
        async with httpx.AsyncClient(timeout=25.0) as client:
            ...
            if ploc.get("lat") is not None and ploc.get("lng") is not None:
                ...
                dlng = math.radians(ploc["lng"] - lng)
                a = (math.sin(dlat / 2) ** 2
                     + math.cos(math.radians(lat)) * math.cos(math.radians(ploc["lat"]))
                     * math.sin(dlng / 2) ** 2)
                offset_m = 2 * 6_371_000 * math.asin(math.sqrt(a))
                if offset_m > 20:
                    logger.info(
                        "site-context: nearest Street View is %.0f m away (>20 m) at "
                        "%.5f,%.5f — skipping grounding", offset_m, lat, lng,
                    )
                    _CACHE[cache_key] = (time.monotonic(), None)
                    return None
            ...
            async def sv(heading: int) -> bytes | None:
                r = await client.get(
                    "https://maps.googleapis.com/maps/api/streetview",
                    params={"pano": pano, "size": "640x640", "heading": heading,
            ...
            async def satellite() -> bytes | None:
                r = await client.get(
                    ...
                    params={"center": f"{lat},{lng}", "zoom": 19, "size": "640x640",
