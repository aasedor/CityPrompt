...
from app.services.master_plan_2d import (
...
def _sample_payload():
    ...
    return {
        ...
        "roads": [
            {
                "geometry": LineString([(-114.0709, 51.04495), (-114.0691, 51.04495)]),
                "kind": "line",
                "width_m": 8.0,
                "properties": {"road_type": "collector"},
            },
        ],
        "paths": [
            {
                "geometry": LineString([(-114.07055, 51.0441), (-114.07055, 51.0453)]),
                "kind": "path",
                "width_m": 4.0,
                "properties": {"road_type": "pedestrian path"},
            },
        ],
        "parks": [
            {
                ...
                "name": "Central Park",
                "properties": {"tree_density_level": "dense", "tree_density": 0.3, "has_paths": True},
            },
        ],
        "plazas": [
            {
...
def test_variants_keep_geometry_locked_and_change_style_only():
    ...
    version_a = _render_variant_assets(
        "Renderer Test",
        scene,
        "rendered_sales_plan",
        _toggles(),
        "greener park",
        ["https://example.com/archetype.png"],
        reference_metadata,
        "board_ready",
        0,
        False,
    ...
    version_b = _render_variant_assets(
        "Renderer Test",
        scene,
        "rendered_sales_plan",
        _toggles(),
...
def test_precinct_generator_creates_multiple_footprints_inside_zone():
    ...
    context = {
        "roads": [
            {
                "geometry": LineString([(-114.0713, 51.04545), (-114.0689, 51.04545)]),
                "kind": "line",
                "width_m": 10.0,
                "properties": {"road_type": "collector"},
            },
        ...
        "paths": [
            {
                "geometry": LineString([(-114.0708, 51.0441), (-114.0708, 51.0455)]),
                "kind": "path",
                "width_m": 4.0,
                "properties": {"road_type": "pedestrian path"},
            },
    ...
    for item in footprints:
        assert zone_geometry.buffer(1e-9).contains(item["geometry"]) or zone_geometry.buffer(1e-9).covers(
            item["geometry"]
        )
...
def test_tree_layout_stays_stable_when_style_variants_change():
    ...
    assert [(round(x, 3), round(y, 3)) for x, y, *_ in points_a] == [
        (round(x, 3), round(y, 3)) for x, y, *_ in points_b
    ]
...
def test_style_pass_prompt_uses_professional_planimetric_language():
    ...
    assert (
        "Treat the entire site as one continuous sheet with one palette family, one light direction, and one shared paper texture."
        in prompt
    )
    assert (
    ...
    assert (
        "[Within #9b59b6 Zone]: Render the roof plan and building footprint for Contemporary Mid-Rise Residential."
        in prompt
    )
...
def test_build_master_plan_3d_response_preserves_geometry_and_metadata():
    ...
    option = SimpleNamespace(
        id=uuid.uuid4(), label="Version A", metadata_={"prompt": "Focus on warm, premium mixed-use realism"}
    )
    request = MasterPlan3DGenerateRequest(
    ...
    assert package["render_prompt"].startswith(
        "A premium photorealistic architectural 3D rendering from a street-level eye-height perspective."
    )
    assert (
        "Critical Directive: Emphasize elegant retail frontages and a stronger corner condition."
        in package["render_prompt"]
    )
    assert (
        package["archetype_metadata"]["generation_style_input"]["aestheticCategoryLabel"]
        == "Contemporary Urban Main Street"
    )
...
def test_render_plan_image_preserves_underlay_through_site_center():
    ...
    rendered = master_plan_2d._render_plan_image(
        ...
        variant=master_plan_2d._make_variant_profile(
            "rendered_sales_plan",
            "board_ready",
            0,
            scene["geometry_hash"],
