...
async def test_context_revision_contract(mode, client, mock_db, test_user, auth_headers, monkeypatch):
    ...
    zone = SimpleNamespace(id=uuid.uuid4(), project_id=uuid.uuid4(), zone_type="site_boundary",
                           geometry=None, properties={"terrain_elevation_m": 1102}, updated_at=before)
    ...
    fetch = AsyncMock(return_value={"buildings": [], "roads": [], "parks": [], "water": [], "fetched_at": before.isoformat()})
