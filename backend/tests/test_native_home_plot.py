...
def test_fixed_landmark_axes_survive_server_replay_without_repeating_homes(degrees):
    ...
    ring = [
        ...
        for x, y in [(-7, -10), (7, -10), (7, 10), (-7, 10)]
    ]
    ...
    result = _strict_locked_building_plan(
        [descriptor_from_library_entry(clay_entry("amsterdam_brown_cafe", "brown_cafe_corner"))],
        "brown_cafe_corner",
        target,
        props,
        zone=zone,
...
def test_half_turn_of_identical_native_plot_invalidates_source_hash():
    ...
    flipped = Polygon(ring[2:] + ring[:2])
    assert original.equals(flipped)
    for flag in ("native_home_plot", "native_plot_axes"):
        props = {flag: True, "floors": 2}
        assert community_3d_source_hash("building", original, props) != community_3d_source_hash(
            "building", flipped, props
        )
