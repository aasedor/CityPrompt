...
from app.services.lego_assembly import AssemblyPlanningError, descriptor_from_library_entry, detached_plot_local_coordinates
...
def test_home_plot_is_opt_in_and_never_repeats_landmarks_or_invents_variant():
    ...
    with pytest.raises(AssemblyPlanningError, match='Only detached-home'):
        plan(clay_entry('amsterdam_brown_cafe', 'brown_cafe_corner'), native_home_plot=True, archetype_id='brown_cafe_corner')
    with pytest.raises(AssemblyPlanningError):
...
def test_server_replay_uses_persisted_home_plot_setting():
    ...
    actual=_strict_locked_building_plan([descriptor_from_library_entry(clay_entry())], 'infill_flat_roof_minimal', (56,33,2,'rectangle',None),props,zone=zone)
    assert actual['fit']['dwelling_count'] > 1
    assert community_3d_source_hash('building',geometry,props) != community_3d_source_hash('building',geometry,{**props,'native_home_plot':False})
    assert community_3d_source_hash('building',geometry,{**props,'native_home_plot':False}) == community_3d_source_hash('building',geometry,{k:v for k,v in props.items() if k!='native_home_plot'})
...
def test_home_frontage_survives_resize_rotation_and_server_replay(width, degrees):
    ...
    zone = SimpleNamespace(geometry=from_shape(Polygon(ring),srid=4326),
        properties={'native_home_plot':True,'floors':2})
