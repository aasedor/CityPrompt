...
@pytest.mark.anyio
async def test_wave10_fire_station_api_plans_parent_variant_and_streetwall(
    client,
    mock_db,
    test_user,
    auth_headers,
...
@pytest.mark.anyio
async def test_wave10_hospital_api_plans_parent_aliases_and_streetwall(
    client,
    mock_db,
    test_user,
    auth_headers,
    ...
    assert regional.json()["family"] == family
    assert oversized.status_code == 200, oversized.text
    oversized_plan = oversized.json()
    assert oversized_plan["family"] == family
    assert oversized_plan["fit"]["compatibility_source"] == ("streetwall_repeat")
...
@pytest.mark.anyio
async def test_wave10_gas_station_api_plans_parent_aliases_and_streetwall(
    client,
    mock_db,
    test_user,
    auth_headers,
...
@pytest.mark.anyio
async def test_wave10_retail_strip_api_plans_parent_aliases_and_streetwall(
    client,
    mock_db,
    test_user,
    auth_headers,
...
@pytest.mark.anyio
async def test_wave10_motor_inn_api_plans_parent_aliases_and_streetwall(
    client,
    mock_db,
    test_user,
    auth_headers,
...
@pytest.mark.anyio
async def test_wave10_daylight_factory_api_plans_aliases_and_streetwall(
    client,
    mock_db,
    test_user,
    auth_headers,
...
@pytest.mark.anyio
async def test_wave10_corten_library_api_plans_aliases_and_streetwall(
    client,
    mock_db,
    test_user,
    auth_headers,
...
@pytest.mark.anyio
@pytest.mark.parametrize(
    ("family", "variant_size", "oversized_width", "footprint_profile"),
    [
        ("coastal-mediterranean-resort", (36.0, 25.0), 120.0, "rectangle"),
        ("skyline-glass-office-cluster", (63.0, 36.0), 210.0, "rectangle"),
        (
...
async def test_wave11_diverse_api_plans_parent_variant_and_oversized_streetwall(
    client,
    mock_db,
    test_user,
    auth_headers,
    ...
    variant = await client.post(
        ...
        headers=auth_headers,
        json={
            "target_width_m": variant_size[0],
            "target_depth_m": variant_size[1],
            "target_floors": manifest["native_floors"],
    ...
    assert parent.json()["family"] == family
    assert variant.status_code == 200, variant.text
    assert variant.json()["family"] == family
    assert oversized.status_code == 200, oversized.text
    oversized_plan = oversized.json()
    assert oversized_plan["family"] == family
    assert oversized_plan["fit"]["compatibility_source"] == "streetwall_repeat"
...
@pytest.mark.parametrize(
    ("family", "variant_size", "oversized_width", "footprint_profile"),
    [
        ("vertical-forest-residential", (28.8, 28.2), 104.0, "courtyard"),
        ("collegiate-gothic-gatehouse", (56.0, 46.0), 198.0, "u_shape"),
        ("cable-stayed-airport-terminal", (80.0, 66.0), 280.0, "l_shape"),
    ],
)
async def test_wave12_diverse_api_plans_parent_variant_and_oversized_streetwall(
    client,
    mock_db,
    test_user,
    auth_headers,
...
@pytest.mark.parametrize(
    ("family", "variant_size", "oversized_width", "footprint_profile"),
    [
        ("art-deco-cream-terracotta-tower", (29.5, 29.0), 108.0, "rectangle"),
        ("restored-kyoto-machiya", (18.5, 15.8), 72.0, "l_shape"),
        ("mid-century-glass-steel-pavilion", (27.0, 21.0), 98.0, "l_shape"),
        ("timber-glass-transit-station-block", (58.0, 39.0), 196.0, "l_shape"),
        ("passive-house-timber-block", (35.0, 21.0), 126.0, "l_shape"),
    ],
)
async def test_wave13_diverse_api_plans_parent_variant_and_oversized_streetwall(
    client,
    mock_db,
    test_user,
    auth_headers,
    ...
    assert parent.status_code == 200, parent.text
    assert parent.json()["family"] == family
    assert parent.json()["fit"]["compatibility_source"] != "family_incompatible"
    assert variant.status_code == 200, variant.text
    assert variant.json()["family"] == family
    assert variant.json()["fit"]["compatibility_source"] != "family_incompatible"
    assert oversized.status_code == 200, oversized.text
    oversized_plan = oversized.json()
    assert oversized_plan["family"] == family
    assert oversized_plan["fit"]["compatibility_source"] == "streetwall_repeat"
...
@pytest.mark.anyio
@pytest.mark.parametrize(
    ("family", "variant_size", "oversized_width"),
    [
        ("art-deco-black-chrome-tower", (34.0, 29.0), 120.0),
        ("art-deco-polychrome-zigzag-tower", (43.0, 38.0), 150.0),
        ("mid-century-wood-stone-pavilion", (39.0, 25.0), 130.0),
        ("mid-century-white-brise-soleil-pavilion", (30.0, 23.0), 105.0),
        ("modern-black-screen-machiya", (13.0, 18.0), 55.0),
        ("red-machiya-cafe-gallery", (18.0, 15.0), 65.0),
        ("dark-frame-clear-glass-office", (34.0, 25.0), 120.0),
        ("mass-timber-glass-office", (43.0, 27.0), 150.0),
        ("streamline-moderne-theater", (26.0, 31.0), 95.0),
        ("egyptian-revival-theater", (30.0, 35.0), 105.0),
    ],
)
async def test_wave14_variant_api_plans_in_band_and_oversized_streetwall(
    client,
    mock_db,
    test_user,
    auth_headers,
    ...
    assert in_band.status_code == 200, in_band.text
    assert in_band.json()["family"] == family
    assert in_band.json()["fit"]["compatibility_source"] != "family_incompatible"
    assert oversized.status_code == 200, oversized.text
    oversized_plan = oversized.json()
    assert oversized_plan["family"] == family
    assert oversized_plan["fit"]["compatibility_source"] == "streetwall_repeat"
...
@pytest.mark.anyio
async def test_multifamily_piece1_timber_glass_api_contract(
    client,
    mock_db,
    test_user,
    auth_headers,
    ...
    parent = await client.post(
        "/api/v1/lego-assembly/plan",
        headers=auth_headers,
        json={**native_request, "archetype_id": manifest["archetype_id"]},
    )
    variant = await client.post(
        "/api/v1/lego-assembly/plan",
        headers=auth_headers,
        json={**native_request, "archetype_id": manifest["variant_id"]},
    )
    oversized = await client.post(
        "/api/v1/lego-assembly/plan",
        headers=auth_headers,
