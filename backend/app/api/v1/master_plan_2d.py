...
async def _load_project_geometry(project_id: uuid.UUID, db: AsyncSession) -> tuple[list[SiteZone], list[Building]]:
    zones_result = await db.execute(select(SiteZone).where(SiteZone.project_id == project_id).order_by(SiteZone.sort_order, SiteZone.created_at))
...
async def generate_options(project_id: uuid.UUID, request: MasterPlan2DGenerateRequest, user: User = Depends(require_auth), db: AsyncSession = Depends(get_db)):
    project = await _get_project(project_id, db)
    ...
    return MasterPlan2DGenerateResponse(project_id=str(project_id), options=[_option_response(option) for option in options])


@router.post('/projects/{project_id}/regenerate', response_model=MasterPlan2DGenerateResponse)
async def regenerate_options(project_id: uuid.UUID, request: MasterPlan2DGenerateRequest, user: User = Depends(require_auth), db: AsyncSession = Depends(get_db)):
...
async def select_option(option_id: uuid.UUID, user: User = Depends(require_auth), db: AsyncSession = Depends(get_db)):
    ...
    return MasterPlan2DSelectResponse(status='selected', project_id=str(selected.project_id), selected_option_id=str(selected.id))


@router.post('/options/{option_id}/generate-3d', response_model=MasterPlan3DGenerateResponse)
async def generate_option_3d_packages(option_id: uuid.UUID, request: MasterPlan3DGenerateRequest, user: User = Depends(require_auth), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(MasterPlan2DOption).where(MasterPlan2DOption.id == option_id))
    option = result.scalar_one_or_none()
    if option is None:
        raise HTTPException(status_code=404, detail='2D master plan option not found')
    ...
    try:
        payload = await MasterPlan2DService.generate_3d_render_packages(db, project, option_id, zones, buildings, request)
        return MasterPlan3DGenerateResponse(**payload)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:
        logger.exception('3D package generation failed for option %s', option_id)
        raise HTTPException(status_code=422, detail='Failed to prepare 3D render packages due to invalid geometry or metadata payload.') from exc

@router.get('/options/{option_id}/export', response_model=MasterPlan2DExportResponse)
async def export_option(option_id: uuid.UUID, width: int = Query(4200, ge=3000, le=5000), user: User = Depends(require_auth), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(MasterPlan2DOption).where(MasterPlan2DOption.id == option_id))
    option = result.scalar_one_or_none()
    if option is None:
        raise HTTPException(status_code=404, detail='2D master plan option not found')
