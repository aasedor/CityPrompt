...
async def list_reference_layers(project_id: uuid.UUID, user: User = Depends(require_auth), db: AsyncSession = Depends(get_db)):
    permission = await check_project_permission(project_id, user, db)
    result = await db.execute(select(ReferenceLayer).where(ReferenceLayer.project_id == project_id).order_by(ReferenceLayer.created_at))
...
async def import_reference_layer(
    ...
    try:
        metadata = ReferenceLayerMetadata(name=(name or PurePosixPath(filename).stem).strip(), kind=kind,
                                          source_url=source_url or None, description=description or None)
    except ValidationError as exc:
        raise HTTPException(422, detail="Provide a name (1–160 characters), a reference/zoning type and an optional valid source URL.") from exc
    ...
    content_hash = hashlib.sha256(json.dumps(parsed.feature_collection, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode()).hexdigest()
    ...
    if duplicate:
        ...
        return duplicate
    if len(existing) >= MAX_PROJECT_LAYERS or sum(layer.storage_bytes for layer in existing) + parsed.storage_bytes > MAX_PROJECT_BYTES:
        raise HTTPException(409, detail="This project supports 20 reference layers and 12 MB of reference data. Remove a layer or import a smaller area.")
    layer = ReferenceLayer(
        project_id=project_id, created_by=user.id, name=metadata.name, kind=metadata.kind,
        source_filename=filename, source_crs=parsed.source_crs[:255],
        source_url=str(metadata.source_url) if metadata.source_url else None, description=metadata.description,
        feature_collection=parsed.feature_collection, feature_count=parsed.feature_count,
        storage_bytes=parsed.storage_bytes, bounds=parsed.bounds, warnings=parsed.warnings,
        content_hash=content_hash, color=metadata.color, opacity=metadata.opacity,
    )
    db.add(layer)
    await db.flush()
...
async def update_reference_layer(layer_id: uuid.UUID, metadata: ReferenceLayerMetadata, user: User = Depends(require_auth), db: AsyncSession = Depends(get_db)):
...
async def delete_reference_layer(layer_id: uuid.UUID, user: User = Depends(require_auth), db: AsyncSession = Depends(get_db)):
    layer = await db.get(ReferenceLayer, layer_id)
    if layer is None:
        raise HTTPException(404, detail="Reference layer not found.")
    await check_project_permission(layer.project_id, user, db, required="editor")
