...
class EvaluationReport(BaseModel):
    ...
    def finalize(self) -> "EvaluationReport":
        ...
        self.overall = (
            round(sum(s.score * SCORE_WEIGHTS.get(k, 1.0) for k, s in self.scores.items()) / total, 3) if total else 0.0
        )
        return self
...
def evaluate_plan(
    ...
    report.add(
        ScoreItem(
            key="fire_access",
            score=1.0 if clear >= FIRE_CLEAR_WIDTH_M else 0.0,
            measured=clear,
            target=FIRE_CLEAR_WIDTH_M,
    ...
    report.add(
        ScoreItem(
            key="open_space",
            score=min(actual_share / target_share, 1.0) if target_share else 1.0,
            measured=round(actual_share, 3),
            target=target_share,
    ...
    if not streets_locked:
        ...
        report.add(
            ScoreItem(
                ...
                score=min(density / INTERSECTION_TARGET_PER_KM2, 1.0),
                measured=round(density, 1),
                target=INTERSECTION_TARGET_PER_KM2,
    ...
    context_total = float(gi.get("context_road_anchors") or 0.0) + float(gi.get("context_path_anchors") or 0.0)
    if context_total:
        context_served = float(gi.get("context_road_connections") or 0.0) + float(
            gi.get("context_path_connections") or 0.0
        )
        share = min(context_served / context_total, 1.0)
        report.add(
            ScoreItem(
                key="context_connectivity",
                score=share,
    ...
    report.add(
        ScoreItem(
            key="canopy_proxy",
            score=min(canopy_share / CANOPY_TARGET, 1.0),
            measured=round(canopy_share, 3),
    ...
    if unit_target and units_estimate is not None:
        report.add(
            ScoreItem(
                key="yield_vs_target",
                score=min(units_estimate / unit_target, 1.0),
                measured=round(units_estimate),
                target=unit_target,
    ...
    report.add(
        ScoreItem(
            key="ceiling_conformance",
            score=1.0,
            detail="storeys clamped to district ceilings during massing"
            + (
                f" ({clamped} clamp note{'s' if clamped != 1 else ''})"
                if clamped
                else "; districts without numeric ceilings are disclosed in the metrics reconciliation"
            ),
        )
    )
    ...
    if yield_score and yield_score.score < 0.9:
        ...
        if bumps < MAX_FLOORS_BUMP:
            ...
            record(
                "floors",
                base_floors,
                base_floors + REVISION_FLOOR_STEP,
