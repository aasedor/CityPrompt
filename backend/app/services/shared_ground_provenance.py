...
def _same_ring(left, right) -> bool:
    ...
    return len(a) == len(b) and all(isclose(x, y, rel_tol=0, abs_tol=1e-12) for p, q in zip(a, b) for x, y in zip(p, q))
...
def bind_shared_ground_snapshot(snapshot: SharedGroundSnapshot, zones: Iterable[Any]) -> dict:
    ...
    active = [
        zone for zone in zones if zone.zone_type == "site_boundary" and bool(getattr(zone, "is_active_boundary", False))
    ]
    ...
    if not valid:
        raise HTTPException(
            status_code=409,
            detail="The retained site boundary changed; refresh the project and capture its ground alignment again.",
        )
