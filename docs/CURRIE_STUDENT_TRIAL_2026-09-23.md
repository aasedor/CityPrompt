...
- The saved-render **Download render** link triggered a browser download, but the browser canceled it on this host. `C:\Users\andre\Downloads` is a junction to `D:\C-drive-archive\Downloads`, and D: is not mounted. This identifies a local download destination problem; the saved images remain available in Project Renders. The data-URL download from the free exact-view preview still needs confirmation on a host with a working download directory.
...
2. **PNG download:** Recheck both the saved-render and free exact-view downloads on a machine with a valid Downloads directory. This workstation's broken junction prevents a reliable end-to-end result.
