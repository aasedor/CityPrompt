...
`987a7d31-ef62-4bd6-b163-1eb63342044a`. Final recipe hash:
...
| C4 freshness | PASS for endpoint edit | Undo/Redo changed endpoint and recipe hash; the final compiled recipe survived reload. Pending tile/result races were not exercised. |
...
| C6 editing | PASS for tested actions | Site draw, street placement, selection, endpoint drag, Undo, Redo and reload used normal desktop controls. |
...
| C8 visibility/capture | PASS for exact export; visual review OPEN | Overhead and two oblique views plus free exact preview show the street and parcel. The large level pad has a visible edge; multiple junction-height views and advertised furniture/parked-car comparison are still needed. No AI image was generated. |
...
| S2 network | PARTIAL | An endpoint reaches the northern public road in plan. Bent route, intersection pavement, crossings and T-junction behavior were not tested. |
...
| S4 clearance/identity | NOT TESTED | Tree spacing is visible, but junction clearance, lamp/parked-car identity and pedestrian clear width need a closer review. |
...
and ornamental lamps. This pilot did not establish that those details appear
in the exact 3D model, so exact appearance approval stays open.
...
`irregular-street-zones.json` is the final read-only saved-zone snapshot
...
The next focused review should inspect the northern junction from a low side
view, confirm the level-pad boundary treatment and compare the exact model
against the variant's advertised cars/lamps. Then repeat a route bend and a
pedestrian approach on this irregular plot. This existing-variant pilot does
not satisfy the separate new-variant catalogue entry or publication gates.
