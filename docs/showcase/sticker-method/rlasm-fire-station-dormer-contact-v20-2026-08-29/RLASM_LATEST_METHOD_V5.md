...
## Building-level material-family authority

Fire-station v17 exposed a failure that role-by-role exact-source materials do
not prevent: the ashlar wall, dressed trim, wall core, opening returns and
identity plaque can each be individually plausible yet read as unrelated stone
families. RLASM v5 now requires a building-level material-family contract:

- When the user approves a bounded material-bearing identity or reference
  detail, record it as chromatic and finish authority for every exterior role
  that is physically the same substance. The fire-service crest authorized one
  warm aged limestone family for the complete masonry envelope.
- Assign one material-family ID to wall core, façade skins, side/rear skins,
  reveals, jamb/head/sill returns, quoins, arches, strings, cornices, dormer
  masonry, masonry roof attachments, columns and the identity carrier. Audit
  the family ID in the exported scene; do not infer unity from material names.
- Preserve role morphology within that family. Coursed ashlar may retain joints
  and rougher bump while dressed or carved limestone uses finer grain, lower
  bump and relief, but hue, aging and mineral character remain visibly related.
- A trim value shift is allowed only when it reads as a dressed finish of the
  same stone. Pale-grey or white trim, carriers or returns are a second family
  unless the locked source proves a real material change.
- Do not recolour genuinely different construction to satisfy the family
  contract. Slate, oak, copper, iron and glazing remain their locked materials.
- Review the contract across neutral front, both sides, rear, aerial, façade
  close and oblique glass close. A front-only colour match fails if dormer
  cheeks, cupola columns, rear returns or side openings regress to another
  stone.
- Compare the approved material-bearing reference directly beside the rendered
  façade close on the phone board. Zero generic fallbacks and valid source
  hashes are audit prerequisites, not proof of chromatic family continuity.
- Preserve every partial pass. Fire-station v16 corrected the wall skins but
  retained pale-grey dressed stone; v17 still failed because its source sheet
  remained visibly cool and its side/rear cameras cropped the envelope. V18
  passed only after generating a shadow-neutral dressed-limestone specimen
  from the user-approved crest, assigning it to every exposed masonry role,
  and reframing all four views from grade to finial.

## Roof-mounted opening contact sections

Fire-station v18 passed its bounded material review but a later side close-up
showed that the side dormer windows were complete assemblies floating outside
the mansard. V19 moved the dormer bodies into the roof, but its inherited wall
cutter was shallower than the new body and left a blank limestone skin over
the sash. V20 establishes the reusable roof-opening rule:

- Derive the dormer datum from the actual sloped roof plane at the opening
  height. Never position a roof opening from the nominal eave boundary alone.
- The lower dormer cheeks must cross the roof plane far enough to form a
  credible structural and weathering intersection. A framed window touching
  only empty air remains a P0 contact failure.
- Cut every opaque owner through its full physical depth. Compare cutter depth
  with the thickest wall, skin, or integrated dormer body; a named Boolean is
  not proof that the exterior face is open.
- Build the section from outside inward: closed roof-matched or wall-matched
  cheek/gable, visible jamb/head/sill return, inset sash, optical pane, then a
  separately recessed occupied-depth layer.
- Seat the dormer roof cap on closed walls, bound its ridge/eaves to the body,
  and give the roof intersection an explicit apron or flashing owner. Reject
  black slots, unsupported caps, floating ridges, or unbounded sheet edges.
- Prove contact in both complete side elevations, an aerial, and a slightly
  oblique contact close that shows roof penetration and the opening depth in
  the same frame. Metadata and a front-only render cannot certify contact.
- Preserve the floating and partial-cut revisions as rejected evidence. Fix
  the constructor in a new bounded candidate, then repeat the complete camera
  and independent zero-P0 review.
