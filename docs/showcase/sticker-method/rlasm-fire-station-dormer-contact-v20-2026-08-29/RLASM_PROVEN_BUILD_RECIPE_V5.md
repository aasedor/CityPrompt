...
## Building-level material-family pass

Run this pass after the role sheets are source-locked and before keeper review:

1. Identify the authoritative material-bearing reference. If the user approves
   a crest, plaque, column or other bounded detail as the material target,
   record that asset and hash as the building-level colour/finish authority.
2. Enumerate every exterior object made from the same physical substance:
   structural wall/core, mapped skins, opening returns, quoins, arches,
   cornices, dormers, masonry roof attachments, columns and identity carrier.
3. Assign one machine-readable material-family ID to those materials and assert
   that no enumerated object is missing it.
4. Keep semantic finish variants: coursed ashlar, dressed stone and carved
   relief can use different morphology, bump and roughness, but must share the
   authoritative hue, aging and mineral character. Do not introduce generic
   white trim for contrast.
5. Leave distinct materials distinct. Roof slate, timber, copper, iron and
   glass do not inherit the limestone family.
6. Render the complete evidence set. Inspect front, both sides, rear, aerial,
   façade close and glass close for exposed cores, returns or attachments that
   still use a previous material.
7. Build a 1080×1920 phone board that places the authority beside the mounted
   close-up and includes a before/after view. Fail locally when any exterior
   masonry role reads as a second family.
8. Preserve the partial revision and advance a new version for the correction;
   never overwrite a rendered candidate or promote from metadata alone.

If no existing ornament-free sheet matches the approved material-bearing
reference, generate a new shadow-neutral, orthographic, seamless specimen from
that reference. Preserve the input hash, exact prompt, output hash and bytes;
reject any output containing the identity ornament, joints, borders,
perspective, shadows or façade fragments. Enroll the new sheet explicitly in
the next candidate's fail-closed material list.
...
## Roof-mounted opening recipe

Use this finite sequence for dormers, roof lantern windows, clerestories, and
other occupied openings that interrupt a sloped roof:

1. Solve the roof plane at the opening centre from the constructed roof mesh.
   Record the plane position, slope, eave, ridge, and required penetration.
2. Place a closed wall/cheek volume so its lower section crosses that plane;
   do not mount a finished frame to an exterior card or free-standing box.
3. Measure the complete opaque depth and oversize the wall cutter deliberately
   so it passes through core, cladding, and any integrated dormer body.
4. Inspect the raw cut before adding trim. No residual skin may cover any part
   of the intended pane, sash, or occupied-depth layer.
5. Construct the opening outward-to-inward: material-matched return, recessed
   sash, optical glass, then a distinct deeper occupied room plane.
6. Close the gable or end field, seat the roof cap on its walls, bound every
   ridge/eave, and assign a visible flashing or apron to the roof joint.
7. Render both whole sides, aerial, and a dedicated oblique contact close. The
   close must show wall-to-roof penetration and return-to-glass depth together.
8. Compare against the previous candidate on a 1080×1920 phone board. Preserve
   any floating or partial-cut version and advance a new candidate.
9. Require a separate zero-P0 review for contact, topology, materials,
   visibility, and whole-envelope camera regression before keeper promotion.
