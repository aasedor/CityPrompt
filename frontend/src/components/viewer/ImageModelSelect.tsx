...
export function ImageModelSelect({ value, onChange, disabled = false, availability }: {
  ...
  const pairUnavailable = unavailable('gpt-image-2.5-flare') || unavailable('gpt-image-2.5-sunburst');
  return (
    <div>
      <label className="flex flex-wrap items-center gap-2 text-xs font-semibold">
        ...
        >
          <option value="compare-flare-sunburst" disabled={pairUnavailable}>Flare + Sunburst · 2 images{pairUnavailable ? ' · Awaiting access' : ''}</option>
      ...
      <p className="mt-1 text-[10px] opacity-75">
        {value === 'compare-flare-sunburst'
          ? 'Two image calls, billed separately. Same source view: Flare first, then Sunburst. Both are saved.'
          : pairUnavailable ? 'This account does not yet list both Image 2.5 engines. Choose an available engine.' : 'One image call. Your 3D scene keeps the same geometry controls.'}
      </p>
    </div>
  );
}
