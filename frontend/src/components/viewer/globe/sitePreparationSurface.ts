...
/** Shared base grass tone for prepared sites and missing terrestrial Tiles.
 * This must not be used for the scene/sky background. */
export const SITE_GRASS_BASE_RGB = [102, 133, 69] as const;
export const SITE_GRASS_BACKGROUND = `rgb(${SITE_GRASS_BASE_RGB.join(',')})`;
...
export function createSitePreparationTexture(seed: string, size = 256, appearance: 'earth' | 'grass' = 'earth'): THREE.DataTexture {
  ...
  const earth = appearance === 'grass' ? SITE_GRASS_BASE_RGB : [150, 144, 134];
