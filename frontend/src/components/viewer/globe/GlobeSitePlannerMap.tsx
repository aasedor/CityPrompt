...
import { publicRealmTrialAsset, assertPublicRealmTrialsReady } from './publicRealmTrial';
...
export function GlobeSitePlannerMap({
  ...
  interactionPaused: externalInteractionPaused = false,
  onMeasureModeChange,
  onGlobeReady,
  ...
  useEffect(() => {
    if (!entrancePick) return;
    const zone = siteZones.find(z => z.id === entrancePick.zoneId);
    if (!zone || entrancePickZoneKey(zone) !== entrancePick.zoneKey) { entrancePick.finish(null); return; }
    const cancel = (event: KeyboardEvent) => {
      if (event.key === 'Escape') { event.preventDefault(); event.stopPropagation(); entrancePick.finish(null); }
    };
    window.addEventListener('keydown', cancel, true);
    return () => window.removeEventListener('keydown', cancel, true);
  }, [entrancePick, siteZones]);
    ...
    () => {
      ...
      if (preparedSiteBoundaryIds.size > 0) {
        return [...siteZones.filter((zone) => preparedSiteBoundaryIds.has(zone.id)),
          ...preparedPublicRoadMasks(siteZones)];
      }
  ...
  const captureDirect3D = useCallback((options: {
    ...
    const trackedPromise: Promise<Direct3DCaptureBundle> = (async () => {
      ...
      const restoreContextCamera = options.landscapeBoundary
        ? frameLandscapeContext(camera, options.landscapeBoundary, terrainElevationRef.current, globeControlsRef.current) : null;
      const previousModelsVisible = buildingModelsVisibleRef.current;
      try {
        if (restoreContextCamera) setBuildingModelsVisible(true);
        if (restoreContextCamera) await new Promise<void>(resolve => requestAnimationFrame(() => requestAnimationFrame(() => resolve())));
        ...
        if (restoreContextCamera) setBuildingModelsVisible(previousModelsVisible);
      }
    })().finally(() => {
  ...
  const handleCanvasClick = useCallback((e: MouseEvent) => {
    if (entrancePick) {
      ...
      const zone = siteZones.find(z => z.id === entrancePick.zoneId);
      if (!zone || entrancePickZoneKey(zone) !== entrancePick.zoneKey) { entrancePick.finish(null); return; }
      if (zone.building_id !== candidate?.buildingId || !candidate?.hit) {
        setEntrancePickError('Choose the lowest entrance step on the selected catalogue house.'); return;
      }
  ...
  return (
    <div ref={containerRef} className="relative h-full w-full bg-black" style={{ overflow: 'hidden' }}>
      <Canvas
        ...
        onCreated={({ gl, camera, scene }) => {
          ...
          cleanupCanvasListenersRef.current = () => {
            ...
            cvs.removeEventListener('pointerdown', handlePointerDown, true);
      ...
      >
        ...
        {contextPresentation.visible === 'google' && <GlobeGroundBackdrop />}
