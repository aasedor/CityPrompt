...
export function GlobeParkBatch9Assembly({ familyId, guides, frame, terrainZ }: { familyId: ParkLegoFamilyId; guides: ParkGroundGuide[]; frame: Batch9ProgramFrame; terrainZ: (x: number, y: number) => number }) {
  const lift = PUBLIC_REALM_PROGRAM_BASE_LIFT_METERS; const slug = SKIN_SLUG[familyId as Batch9FamilyId];
  ...
  if (familyId === 'park_cemetery_classical_v0') {
    const cols=Math.min(12,Math.max(4,Math.floor(frame.width/12))); const rows=Math.min(14,Math.max(4,Math.floor(frame.height/11)));
    return <group>{Array.from({length:cols*rows},(_,i)=>{const col=i%cols,row=Math.floor(i/cols);const x=frame.minX+frame.width*(.10+(col+.5)*.80/cols);const y=frame.minY+frame.height*(.10+(row+.5)*.80/rows);if(Math.abs(x-(frame.minX+frame.width/2))<3.5)return null;const z=terrainZ(x,y)+lift;return <mesh key={i} position={[x,y,z+.48]}><boxGeometry args={[.72,.20,.96]} /><meshStandardMaterial {...safetyMaps} color="#c2bbae" roughness={.96} /></mesh>})}<mesh position={[frame.minX+frame.width/2,frame.minY+frame.height/2,terrainZ(frame.minX+frame.width/2,frame.minY+frame.height/2)+lift+.5]}><cylinderGeometry args={[2.2,2.6,1.0,24]} /><meshStandardMaterial {...paverMaps} color="#aaa293" roughness={.9} /></mesh></group>;
  }

  if (familyId === 'park_courtyard_linear_water_v1') {
    const water=guides.find((g)=>g.kind==='polyline');const points=water?route(water,frame):[];
    return <group>{points.slice(0,-1).map((point,i)=><Segment key={i} from={point} to={points[i+1]} width={1.4} height={.10} z={Math.max(terrainZ(point.x,point.y),terrainZ(points[i+1].x,points[i+1].y))+lift+.14} color="#5f858b" />)}{guides.filter((g)=>g.kind==='rectangle').map((guide,i)=>{const c=center(guide,frame);const s=resolveParkGuideDimensionsM(guide,frame);const z=terrainZ(c.x,c.y)+lift;return <group key={i}><mesh position={[c.x,c.y,z+.36]}><boxGeometry args={[s.width,s.height,.72]} /><meshStandardMaterial {...paverMaps} color="#807d75" roughness={.88} /></mesh>{[-.3,.3].map((offset)=><Tree key={offset} x={c.x+s.width*offset} y={c.y} z={z+.72} scale={.58}/>)}</group>})}</group>;
  }
