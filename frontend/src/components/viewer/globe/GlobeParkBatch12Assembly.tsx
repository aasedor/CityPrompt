...
function ConcertShell({x,y,z,w,d,maps,steel=false}:{x:number;y:number;z:number;w:number;d:number;maps:Maps;steel?:boolean}) { return <group position={[x,y,z]}><mesh position={[0,0,.35]}><boxGeometry args={[w*.86,d*.78,.7]}/><meshStandardMaterial {...maps} color="#ffffff" roughness={.9}/></mesh>{[-1,0,1].map((s)=><mesh key={s} position={[s*w*.31,0,3.1-Math.abs(s)*.55]} rotation={[Math.PI/2,0,0]} scale={[w*.24,3.8-Math.abs(s)*.5,1]}><cylinderGeometry args={[1,1,d*.92,32,1,true,0,Math.PI]}/>{steel?<meshPhysicalMaterial color="#d7dcda" metalness={.78} roughness={.24} side={THREE.DoubleSide}/>:<meshStandardMaterial {...maps} color="#ffffff" side={THREE.DoubleSide}/>}</mesh>)}</group>; }
function MarketHall({x,y,z,w,d,maps}:{x:number;y:number;z:number;w:number;d:number;maps:Maps}) { return <group position={[x,y,z]}><mesh position={[0,0,2.1]}><boxGeometry args={[w,d,4.2]}/><meshPhysicalMaterial color="#b9c8c8" transparent opacity={.42} roughness={.18} metalness={.12}/></mesh>{[-1,1].map(s=><mesh key={s} position={[0,s*d*.48,4.5]} rotation={[s*.34,0,0]}><boxGeometry args={[w,1.8,.18]}/><meshStandardMaterial color="#454b4a" metalness={.65} roughness={.32}/></mesh>)}{Array.from({length:Math.max(3,Math.floor(w/5))},(_,i)=><mesh key={i} position={[-w*.4+i*w*.8/Math.max(1,Math.floor(w/5)-1),0,1.2]}><boxGeometry args={[.18,d,2.4]}/><meshStandardMaterial {...maps} color="#ffffff"/></mesh>)}</group>; }
...
export function GlobeParkBatch12Assembly({familyId,archetypeId,variantId,guides,frame,terrainZ}:{familyId:ParkLegoFamilyId;archetypeId?:string;variantId?:string;guides:ParkGroundGuide[];frame:ProgramFrame;terrainZ:(x:number,y:number)=>number}) {
  const id=familyId as FamilyId,selectedSkin=batch26ParkSkinForSelection(archetypeId??'',variantId??'')??batch22ParkSkinForSelection(archetypeId??'',variantId??'')??batch20ParkSkinForSelection(archetypeId??'',variantId??''),slug=selectedSkin?.slug??SLUG[id],variantSkin=selectedSkin!==null,lift=PUBLIC_REALM_PROGRAM_BASE_LIFT_METERS,paver=useMaps(slug,'paver'),planting=useMaps(slug,'planting',5),timber=useMaps(slug,'timber'),safety=useMaps(slug,'safety');
  ...
  if(id==='park_concert_timber_lawn_v2') {
    const stageY=frame.maxY-frame.height*.18,w=Math.min(20,frame.width*.34),d=Math.min(10,frame.height*.18);
    if(variantId==='concert_pavilion_lawn_v0') {
      Array.from({length:4},(_,i)=>extras.push(<mesh key={`tier-${i}`} position={[cx,cy-frame.height*.06+i*frame.height*.055,base+.16+i*.28]}><boxGeometry args={[frame.width*(.56-i*.07),3.2,.38]}/><meshStandardMaterial {...paver} color="#ffffff"/></mesh>));
      extras.push(<ConcertShell key="shell" x={cx} y={stageY} z={base} w={w} d={d} maps={paver}/>);
    } else if(variantId==='concert_pavilion_lawn_v1') extras.push(<ConcertShell key="wave" x={cx} y={stageY} z={base} w={w} d={d} maps={timber} steel/>);
    else if(variantId==='concert_pavilion_lawn_v3') {
      Array.from({length:5},(_,i)=>extras.push(<mesh key={`bowl-${i}`} position={[cx,cy-frame.height*.02+i*frame.height*.045,base+.16+i*.25]}><boxGeometry args={[frame.width*(.60-i*.065),2.6,.34]}/><meshStandardMaterial {...paver} color="#ffffff"/></mesh>));
      extras.push(<ConcertShell key="bowl-shell" x={cx} y={stageY} z={base} w={w*1.05} d={d} maps={paver}/>);
    } else extras.push(<Canopy key="stage" x={cx} y={stageY} z={base} w={Math.min(12,frame.width*.28)} d={Math.min(7,frame.height*.16)} maps={timber}/>);
  }
  if(id==='park_night_market_hawker_v0'){
    if(variantId==='night_market_v3') extras.push(<MarketHall key="hall" x={cx} y={cy} z={base} w={Math.min(28,frame.width*.62)} d={Math.min(16,frame.height*.52)} maps={timber}/>);
    else {const christmas=variantId==='night_market_v1',latin=variantId==='night_market_v2',n=Math.min(christmas?10:8,Math.max(3,Math.floor(frame.width/12)));for(let i=0;i<n;i++){const a=i*Math.PI*2/n,x=christmas?cx+Math.cos(a)*frame.width*.26:frame.minX+frame.width*(.12+i*.76/Math.max(1,n-1)),yA=christmas?cy+Math.sin(a)*frame.height*.29:frame.maxY-frame.height*.22,yB=frame.minY+frame.height*.22;extras.push(<Stall key={`a${i}`} x={x} y={yA} z={terrainZ(x,yA)+lift} flip={christmas?Math.sin(a)<0:false} maps={timber}/>);if(!christmas)extras.push(<Stall key={`b${i}`} x={x} y={yB} z={terrainZ(x,yB)+lift} flip maps={timber}/>);}if(christmas)extras.push(<mesh key="tree" position={[cx,cy,base+3.8]}><coneGeometry args={[2.8,7.6,12]}/><meshStandardMaterial {...planting} color="#ffffff"/></mesh>);if(latin)extras.push(<mesh key="shade" position={[cx,cy,base+4.7]} rotation={[0,0,.08]}><boxGeometry args={[frame.width*.72,frame.height*.62,.12]}/><meshStandardMaterial {...safety} color="#ffffff" transparent opacity={.72}/></mesh>);}
  }
  if(id==='park_parade_national_mall_v3') {
    if(variantId==='parade_ground_v0') {[-.24,0,.24].forEach((d,i)=>extras.push(<mesh key={i} position={[cx+d*frame.width,cy,base+.3]}><boxGeometry args={[Math.min(12,frame.width*.18),Math.min(22,frame.height*.55),.6]}/><meshStandardMaterial {...paver} color="#ffffff"/></mesh>));}
    else if(variantId==='parade_ground_v1') {[-.22,.22].forEach((d,i)=>extras.push(<Segment key={i} a={{x:cx+d*frame.width,y:frame.minY+frame.height*.06}} b={{x:cx+d*frame.width,y:frame.maxY-frame.height*.06}} width={Math.min(7,frame.width*.09)} z={base+.2} maps={paver} color="#ffffff"/>));}
    else if(variantId==='parade_ground_v2') {[-.28,.28].forEach((d,i)=>extras.push(<Segment key={i} a={{x:cx+d*frame.width,y:frame.minY+frame.height*.06}} b={{x:cx+d*frame.width,y:frame.maxY-frame.height*.06}} width={Math.min(4,frame.width*.05)} z={base+.2} maps={paver} color="#ffffff"/>));}
    else extras.push(<Fountain key="water" x={cx} y={cy} z={base+.18} wide/>);
  }
