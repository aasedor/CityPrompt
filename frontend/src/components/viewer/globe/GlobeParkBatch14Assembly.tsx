...
function ParkingField({ frame, z, maps, green = false, retail = false }: { frame: ProgramFrame; z: number; maps: Maps; green?: boolean; retail?: boolean }) {
  ...
  children.push(<FlatBox key="base" x={(frame.minX + frame.maxX) / 2} y={(frame.minY + frame.maxY) / 2} z={z} width={width} depth={depth} maps={maps} color={green ? '#728469' : '#505352'} />);
...
export function GlobeParkBatch14Assembly({ familyId, guides: _guides, frame, terrainZ }: { familyId: ParkLegoFamilyId; guides: ParkGroundGuide[]; frame: ProgramFrame; terrainZ: (x: number, y: number) => number }) {
  ...
  if (id === 'park_surface_parking_standard_v0' || id === 'park_green_parking_infrastructure_v1' || id === 'park_retail_parking_landscaped_v1') {
    items.push(<ParkingField key="parking" frame={frame} z={base} maps={id === 'park_green_parking_infrastructure_v1' ? paver : asphalt} green={id === 'park_green_parking_infrastructure_v1'} retail={id === 'park_retail_parking_landscaped_v1'} />);
    ...
    if (id === 'park_green_parking_infrastructure_v1') {
    ...
    if (id === 'park_retail_parking_landscaped_v1') {
  ...
  if (id === 'park_structured_parking_urban_v2') {
    items.push(<FlatBox key="garage-reservation" x={cx} y={cy + frame.height * .06} z={base} width={frame.width * .72} depth={frame.height * .62} maps={paver} color="#a19d95" height={.32} />);
    items.push(<ParkingField key="arrival" frame={{ ...frame, maxY: frame.minY + frame.height * .25, height: frame.height * .25 }} z={base} maps={asphalt} />);
  ...
  if (id === 'park_underground_parking_green_v1') {
    items.push(<FlatBox key="landscape" x={cx} y={cy} z={base} width={frame.width * .92} depth={frame.height * .90} maps={lawn} color="#69845b" />);
    ...
    for (let index = 0; index < 5; index += 1) items.push(<Tree key={`tree-${index}`} x={frame.minX + frame.width * (.12 + index * .18)} y={frame.maxY - frame.height * .18} z={base + .2} />);
  }
  ...
  if (id === 'park_airport_general_aviation_v2') {
    ...
    const runwayW = Math.max(12, frame.height * .26);
    items.push(<FlatBox key="runway" x={cx} y={cy - frame.height * .12} z={base + .18} width={frame.width * .86} depth={runwayW} maps={asphalt} color="#494d4e" />);
    const dashes = Math.max(6, Math.min(18, Math.floor(frame.width / 18)));
    for (let index = 0; index < dashes; index += 1) items.push(<Line key={`dash-${index}`} x={frame.minX + frame.width * (.10 + .80 * ((index + .5) / dashes))} y={cy - frame.height * .12} z={base + .38} width={Math.max(2, frame.width / dashes * .45)} depth={.24} />);
    items.push(<FlatBox key="apron" x={frame.minX + frame.width * .28} y={frame.maxY - frame.height * .19} z={base + .18} width={frame.width * .34} depth={frame.height * .20} maps={paver} color="#8d8d88" />);
    items.push(<FlatBox key="hangar-reservation" x={frame.minX + frame.width * .17} y={frame.maxY - frame.height * .08} z={base + .28} width={frame.width * .26} depth={frame.height * .08} color="#b0ada5" />);
    for (let index = 0; index < 3; index += 1) items.push(<Aircraft key={`aircraft-${index}`} x={frame.minX + frame.width * (.20 + index * .09)} y={frame.maxY - frame.height * .20} z={base + .38} rotation={Math.PI / 2} />);
  }
  ...
  if (id === 'park_equestrian_working_stable_v1') {
    ...
    const arenaW = frame.width * .52;
    const arenaD = frame.height * .48;
    ...
    for (let index = 0; index < 3; index += 1) items.push(<FenceRect key={`paddock-${index}`} x={frame.minX + frame.width * (.15 + index * .16)} y={frame.minY + frame.height * .20} z={base + .2} width={frame.width * .13} depth={frame.height * .22} />);
  }

  if (id === 'park_golf_seaside_links_v0') {
    items.push(<FlatBox key="links" x={cx} y={cy} z={base} width={frame.width * .97} depth={frame.height * .95} maps={planting} color="#8a805d" />);
  ...
  if (id === 'park_driving_range_single_tier_v0') {
    ...
    for (let index = 0; index < targetCount; index += 1) {
      ...
      items.push(<mesh key={`target-${index}`} position={[x, y, base + .22]} scale={[2.4 + index * .2, 2.4 + index * .2, 1]}><ringGeometry args={[.55, 1, 28]} /><meshStandardMaterial color={index % 2 ? '#d8b55e' : '#d7d8cf'} side={THREE.DoubleSide} /></mesh>);
    ...
    for (let index = 0; index <= bays; index += 1) {
      ...
      items.push(<mesh key={`post-${index}`} position={[x, bayY, base + 1.8]}><boxGeometry args={[.12, .12, 3.2]} /><meshStandardMaterial {...timber} color="#6d5643" /></mesh>);
    }
    items.push(<mesh key="canopy" position={[cx, bayY, base + 3.35]}><boxGeometry args={[frame.width * .74, frame.height * .13, .18]} /><meshStandardMaterial {...timber} color="#79614b" /></mesh>);
  ...
  if (id === 'park_multi_sport_track_field_v3') {
    ...
    [[-.23, -.32], [.43, -.32], [-.23, .32], [.43, .32]].forEach(([dx, dy], index) => items.push(<LightPole key={`flood-${index}`} x={cx + frame.width * dx} y={cy + frame.height * dy} z={base + .2} height={10} />));
  }
