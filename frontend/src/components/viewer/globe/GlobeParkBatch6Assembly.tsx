...
function PicnicTable({ x, y, z, rotation = 0, timberMaps }: {
  x: number; y: number; z: number; rotation?: number; timberMaps: MaterialMaps;
}) {
  return <group position={[x, y, z]} rotation={[0, 0, rotation]}>
    <mesh position={[0, 0, 0.72]}><boxGeometry args={[2.3, 0.72, 0.12]} /><meshStandardMaterial {...timberMaps} color="#a57a4f" roughness={0.86} /></mesh>
    {[-0.72, 0.72].map((side) => <mesh key={side} position={[0, side, 0.44]}><boxGeometry args={[2.3, 0.32, 0.10]} /><meshStandardMaterial {...timberMaps} color="#95704a" roughness={0.88} /></mesh>)}
...
export function GlobeParkBatch6Assembly({
  ...
  const slug = SKIN_SLUG[familyId as Batch6FamilyId];
  ...
  if (familyId === 'park_ice_rink_multipurpose_v3') {
    ...
    return <group position={[c.x, c.y, z]} rotation={[0, 0, yaw]}>
      <mesh position={[0, 0, 0.035]}><boxGeometry args={[s.width, s.height, 0.07]} /><meshStandardMaterial {...safetyMaps} color="#d7e2e3" roughness={0.54} metalness={0.02} /></mesh>
  ...
  if (familyId === 'park_kayak_river_launch_v0') {
    ...
    return <group>
      <mesh position={[c.x, c.y, z + 0.25]}><boxGeometry args={[s.width, s.height, 0.5]} /><meshStandardMaterial {...timberMaps} color="#8b765f" roughness={0.86} /></mesh>
  ...
  if (familyId === 'park_tidal_marsh_cordgrass_v0') {
    ...
    return <group>
      ...
      {points.map((point, index) => <mesh key={`post-${index}`} position={[point.x, point.y, terrainZ(point.x, point.y) + lift + 0.42]} rotation={[Math.PI / 2, 0, 0]}><cylinderGeometry args={[0.10, 0.12, 0.84, 8]} /><meshStandardMaterial {...timberMaps} color="#6f5d49" roughness={0.9} /></mesh>)}
      <mesh position={[oc.x, oc.y, oz + 0.88]}><cylinderGeometry args={[os.width / 2, os.width / 2, 0.24, 6]} /><meshStandardMaterial {...timberMaps} color="#88745d" roughness={0.88} /></mesh>
      <group position={[points[0]?.x ?? oc.x, points[0]?.y ?? oc.y, terrainZ(points[0]?.x ?? oc.x, points[0]?.y ?? oc.y) + lift]}>
        <mesh position={[0, 0, 1.3]} rotation={[Math.PI / 2, 0, 0]}><cylinderGeometry args={[0.10, 0.12, 2.6, 8]} /><meshStandardMaterial {...timberMaps} color="#6f5d49" /></mesh>
        <mesh position={[0, 0, 2.25]} rotation={[0.55, 0, 0]}><boxGeometry args={[1.6, 0.10, 0.9]} /><meshStandardMaterial {...paverMaps} color="#9c8d73" roughness={0.82} /></mesh>
      </group>
    </group>;
  }
  ...
  if (familyId === 'park_cinema_lawn_projection_v1') {
    ...
    return <group>
      <group position={[sc.x, sc.y, sz]}>
        <mesh position={[0, 0, 0.55]}><boxGeometry args={[ss.width + 2.4, Math.max(1.2, ss.height), 1.1]} /><meshStandardMaterial {...paverMaps} color="#aaa59a" roughness={0.88} /></mesh>
      ...
      <group position={[bc.x, bc.y, bz]}>
        <mesh position={[0, 0, 1.6]}><boxGeometry args={[bs.width, bs.height, 3.2]} /><meshStandardMaterial {...timberMaps} color="#766352" roughness={0.86} /></mesh>
      ...
      {[-0.22, 0, 0.22].map((ratio) => <group key={ratio} position={[frame.minX + frame.width * (0.5 + ratio), frame.maxY - frame.height * 0.70, terrainZ(frame.minX + frame.width * (0.5 + ratio), frame.maxY - frame.height * 0.70) + lift + 0.42]}><mesh><boxGeometry args={[3.4, 0.48, 0.18]} /><meshStandardMaterial {...timberMaps} color="#98704a" /></mesh></group>)}
    </group>;
  }
  ...
  if (familyId === 'park_food_truck_permanent_v1') {
    ...
    return <group>
      ...
      {tables.map(([x,y],index) => { const px=frame.minX+frame.width*x; const py=frame.maxY-frame.height*y; return <PicnicTable key={index} x={px} y={py} z={terrainZ(px,py)+lift} rotation={index%2?Math.PI/2:0} timberMaps={timberMaps} />; })}
  ...
  if (familyId === 'park_campus_meadow_quad_v0') {
    ...
    return <group>
      {Array.from({length:9},(_,index)=>{const angle=index*Math.PI*2/9;const radius=index%2?4.4:3.1;const x=c.x+Math.cos(angle)*radius;const y=c.y+Math.sin(angle)*radius;return <mesh key={index} position={[x,y,terrainZ(x,y)+lift+0.45]} rotation={[index*0.17,index*0.09,index*0.23]} scale={[1.1+(index%3)*0.25,0.8+(index%2)*0.22,0.65+(index%4)*0.16]}><dodecahedronGeometry args={[0.9,0]}/><meshStandardMaterial {...paverMaps} color="#8e8b83" roughness={0.92}/></mesh>;})}
      <mesh position={[c.x,c.y,z+0.04]}><cylinderGeometry args={[5.4,5.4,0.08,28]}/><meshStandardMaterial {...paverMaps} color="#ada38d" roughness={0.91}/></mesh>
    </group>;
  }
  ...
  if (familyId === 'park_velodrome_open_air_v0') {
    ...
    return <group>
      <group position={[c.x,c.y,z+0.7]} rotation={[0,0,yaw]} scale={[s.width/s.height,1,0.22]}>
        <mesh><torusGeometry args={[s.height*0.36,s.height*0.075,18,96]}/><meshStandardMaterial {...safetyMaps} color="#d3c093" roughness={0.78}/></mesh>
        <mesh position={[0,0,-0.22]}><torusGeometry args={[s.height*0.285,s.height*0.026,12,96]}/><meshStandardMaterial {...safetyMaps} color="#69a4b7" roughness={0.72}/></mesh>
      </group>
      <group position={[bc.x,bc.y,bz]}>{Array.from({length:5},(_,step)=><mesh key={step} position={[0,step*0.78,0.28+step*0.34]}><boxGeometry args={[bs.width,1.0,0.56]}/><meshStandardMaterial {...paverMaps} color="#a5a39d" roughness={0.88}/></mesh>)}</group>
      <group position={[frame.minX+frame.width*0.82,frame.maxY-frame.height*0.82,terrainZ(frame.minX+frame.width*0.82,frame.maxY-frame.height*0.82)+lift]}><mesh position={[0,0,3]}><boxGeometry args={[6,5,6]}/><meshStandardMaterial {...paverMaps} color="#74736f" roughness={0.78}/></mesh><mesh position={[0,-2.55,3.7]}><planeGeometry args={[4.4,2.2]}/><meshStandardMaterial color="#1c2627" metalness={0.28} roughness={0.3}/></mesh></group>
    </group>;
  }
