          ...
          <>
            ...
            <div>
              ...
              <input
                type="number"
                step="1"
                value={isFixedSectionStreet({zone_type:zone.zone_type, properties:props}) ? streetSectionWidth({zone_type:zone.zone_type, properties:props}) : props.width ?? config?.defaultProperties.width ?? 10}
                disabled={isFixedSectionStreet({zone_type:zone.zone_type, properties:props})}
                onChange={(e) => setProps((p) => ({ ...p, width: parseFloat(e.target.value) || undefined }))}
                className={panelFieldClass}
              />
              {isFixedSectionStreet({zone_type:zone.zone_type, properties:props}) && <p className="mt-1 text-xs text-slate-600">This section stays {streetSectionWidth({zone_type:zone.zone_type, properties:props})} m wide. Choose another street type to change the section.</p>}
            </div>
          </>
        )}
