...
describe('connection controls',()=>{
  it('preserves the unsaved dialog through pick cancellation and saves an accepted pick only on Save',async()=>{
    ...
    fireEvent.change(screen.getByLabelText('Walkway width (m)'),{target:{value:'1.65'}});
    fireEvent.click(screen.getByRole('button',{name:'Pick entrance step in 3D'}));
    expect(screen.queryByRole('dialog')).toBeNull();expect(onSave).not.toHaveBeenCalled();
    act(()=>onPickEntrance.mock.calls[0][0].finish(null));
    expect(screen.getByLabelText('Walkway width (m)')).toHaveValue(1.65);
    fireEvent.click(screen.getByRole('button',{name:'Pick entrance step in 3D'}));
    const request=onPickEntrance.mock.calls[1][0];
    act(()=>request.finish({anchor:{...request.properties.pedestrian_building_entrance,xM:.9,yM:-8.965,heightAboveBaseM:0,scaleWithPlot:false},status:'unresolved',message:'Move the building farther from the street.'}));
    expect(screen.getByLabelText('Front / back (m)')).toHaveValue(-8.965);
    expect(screen.getByText(/Move the building farther/)).toBeInTheDocument();
    expect(onSave).not.toHaveBeenCalled();
    fireEvent.change(screen.getByLabelText('Left / right (m)'),{target:{value:'1'}});
    expect(screen.queryByText(/Move the building farther/)).toBeNull();
    fireEvent.click(screen.getByRole('button',{name:'Save connections'}));
    await waitFor(()=>expect(onSave).toHaveBeenCalledOnce());
    expect(onSave.mock.calls[0][0]).toMatchObject({unrelated:'preserve',pedestrian_building_entrance:{xM:1,yM:-8.965,widthM:1.65}});
  });
  it('will not save stale dialog values over a plot changed while picking',()=>{
    const onPickEntrance=vi.fn(),onSave=vi.fn();
    const props={zone:house,zones:[house,road],disabled:false,onSave,onClose:vi.fn(),onPickEntrance};
    const view=render(<ConnectionEditor {...props}/>);
    fireEvent.click(screen.getByRole('checkbox',{name:'Link an entrance to a sidewalk'}));
    fireEvent.click(screen.getByRole('button',{name:'Pick entrance step in 3D'}));
    const changed={...house,coordinates:rectangleAt([-114,51],14,20)};
    view.rerender(<ConnectionEditor {...props} zone={changed} zones={[changed,road]}/>);
    act(()=>onPickEntrance.mock.calls[0][0].finish(null));
    expect(screen.getByRole('alert')).toHaveTextContent('This plot changed');
