...
export function ContextControls({ requested, onChange, loading, failed, children, geographic = false }: {
  requested: ContextView; onChange: (view: ContextView) => void; loading: boolean; failed: boolean; children?: ReactNode; geographic?: boolean;
}) {
  return <div className="absolute bottom-16 right-4 z-30 max-w-xs rounded-xl border border-slate-700 bg-white p-3 text-sm text-slate-900 shadow-lg">
    ...
    <p className="mt-1 text-xs">{geographic ? 'Geographic test sample. Surface alignment is under review; design ground stays fixed.' : 'Development sample with an artificial location.'}</p>
