...
export function useContextPresentation(zones: SiteZone[]) {
  ...
  useEffect(() => {
    ...
    if (!import.meta.env.DEV || import.meta.env.VITE_CONTEXT_PILOT !== 'true' || !projectId || !prepared) return;
    const controller = new AbortController();
    void fetch('/context-pilot/manifest.json', { signal: controller.signal }).then(r => r.ok ? r.json() : null)
  ...
  const available = provider?.projectId === projectId && prepared ? provider : null;
