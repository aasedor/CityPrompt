    ...
    <div className="card">
      ...
      <div className="mb-3 flex flex-wrap gap-3">
        {allProviders.map((provider) => (
          <div key={provider} className="flex items-center gap-1.5 text-xs text-gray-600">
            <span className={`inline-block h-2.5 w-2.5 rounded-full ${PROVIDER_DOT_COLORS[provider] || 'bg-gray-400'}`} />
      ...
      {/* CSS Bar Chart */}
      {dates.length === 0 ? (
        <p className="py-8 text-center text-sm text-gray-400">No usage data in this period</p>
      ) : (
                ...
                >
                  {allProviders.map((provider) => {
                    ...
                    return (
                      <div
                        key={provider}
                        className={`w-full ${PROVIDER_COLORS[provider] || 'bg-gray-400'}`}
      ...
      {data.providers.length > 0 && (
        <div className="mt-6">
          <p className="mb-2 text-xs font-medium text-gray-500">Breakdown by Provider & Operation</p>
                ...
                {data.providers.map((provider) =>
                  provider.by_operation.length > 0 ? (
                    provider.by_operation.map((op, i) => (
                      <tr key={`${provider.provider}-${op.operation}`} className="border-b border-gray-50 last:border-0">
                        <td className="py-2 pr-3">
                          {i === 0 ? (
                            <div className="flex items-center gap-1.5">
                              <span className={`inline-block h-2 w-2 rounded-full ${PROVIDER_DOT_COLORS[provider.provider] || 'bg-gray-400'}`} />
                              <span className="font-medium capitalize text-gray-700">{provider.provider}</span>
                            </div>
                          ) : null}
                        </td>
                        <td className="py-2 pr-3 text-gray-600">{op.operation}</td>
                        <td className="py-2 pr-3 text-right text-gray-600">{op.total_credits.toFixed(1)}</td>
                        <td className="py-2 pr-3 text-right text-gray-600">{op.call_count}</td>
                    ...
                    <tr key={provider.provider} className="border-b border-gray-50 last:border-0">
                      <td className="py-2 pr-3">
                        <div className="flex items-center gap-1.5">
                          <span className={`inline-block h-2 w-2 rounded-full ${PROVIDER_DOT_COLORS[provider.provider] || 'bg-gray-400'}`} />
                          <span className="font-medium capitalize text-gray-700">{provider.provider}</span>
                      ...
                      <td className="py-2 pr-3 text-right text-gray-600">{provider.total_credits.toFixed(1)}</td>
                      <td className="py-2 pr-3 text-right text-gray-600">{provider.total_calls}</td>
                      <td className="py-2 text-right">
                        <span
                          className={`font-medium ${
